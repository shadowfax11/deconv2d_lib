...
x_est = restoration.richardson_lucy(b, h)
...
ax3.set_title('Scene Reconstructed\nSNR={:2.3f}dB, SSIM={:.3f}'.format(calc_snr(x_est, x),ssim(x_est, x)))
fh1.colorbar(im3, ax=ax3)
print("Image restored, SNR=%3.3f dB, SSIM=%2.3f"%(calc_snr(x_est, x),ssim(x_est, x)))
plt.show()
